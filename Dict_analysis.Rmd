...
```{r, eval=FALSE}
nasalization <- "̃"
vowels <- str_c("[auieo]", nasalization, "?ː?\\*?")
godoberi_ipa %>% 
  mutate(initial = str_extract(IPA, str_c("^.*?", vowels)),
         ...
         initial_length = str_count(initial, "-")+1) %>% 
  #        final = str_extract(IPA, "[auieo][^auieo]*$"),
  #        final_vowel = str_extract(final, "[auieo]"),
  #        final = ifelse(str_detect(final, "-"), 
  #                       str_remove(final, "[auieo].*?-"), 
  #                       "#")) %>% 
  View()
```
